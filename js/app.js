...
window.PLACEHOLDER_IMG = 'data:image/svg+xml;utf8,' + encodeURIComponent(
  ...
  '<rect width="400" height="300" fill="#0c162c"/>' +
  '<rect x="0.5" y="0.5" width="399" height="299" fill="none" stroke="#2a3a5a"/>' +
  '<circle cx="200" cy="132" r="34" fill="none" stroke="#9ec9ff" stroke-width="1.5" stroke-dasharray="6 5"/>' +
...
const PUBS = [
  { 
    ...
    subInst: "Media Lab · Responsive Environments · 2025 · Dr. Joseph Paradiso",
  ...
  { 
    ...
    subInst: "NASA JPL · Oral presentation, AGU 2024 · paper in progress · Dr. Thomas Lu",
  ...
  { 
    ...
    inst: "NASA JET PROPULSION LABORATORY", 
    subInst: "NASA JPL · Full paper, IAC 2024 · Dr. Thomas Lu",
  ...
  { 
    ...
    subInst: "ESA European Astronaut Centre · Confidential manuscript, 2022 · Dr. Igor Drozdovsky",
  ...
  { 
    ...
    inst: "NASA JET PROPULSION LABORATORY", 
    subInst: "NASA JPL · Abstract, AGU 2024 · Dr. Thomas Lu",
  ...
  { 
    ...
    inst: "FRONTIER DEVELOPMENT LAB (FDL)",  
    subInst: "FDL · Oral presentation, AGU 2024 · Dr. Andrés Muñoz, Dr. R Jarolim, Dr. B Tremblay",
  ...
  { 
    ...
    inst: "FRONTIER DEVELOPMENT LAB (FDL)",  
    subInst: "FDL · Interactive presentation, AGU 2024 · Dr. Andrés Muñoz, Dr. R Jarolim, Dr. B Tremblay",
...
const ABOUT_ME = {
  ...
  extendedDesc: "I'm a PhD student at MIT in the Department of Aeronautics and Astronautics, advised by Dr. Dava Newman and Dr. Daniel Varon, working on artificial intelligence for remote sensing analysis and outer space exploration. An AI and Aerospace Engineer with an industry background in software and test engineering, I also hold a second Master's specialized in Artificial Intelligence from Mines Paris – PSL University.<br><br>I joined MIT after four years designing and deploying AI algorithms for outer space exploration at ESA European Astronaut Centre, NASA Jet Propulsion Laboratory, Frontier Development Lab (FDL) and Space Cargo Unlimited. At JPL I worked on physics-informed machine learning for lunar rover motion prediction and computer vision for SWOT satellite imagery, which earned a JPL Spotlight Technology Award; at FDL I contributed to the first AI foundation model for Moon exploration (Lunar-FM). My research combines deep learning, computer vision, physics-informed machine learning, and systems engineering to solve complex challenges in space science and exploration.",
...
function showDetail(item) {
  ...
  // Centre on the related object. This used to pause the sim too, which read
  // as a broken page rather than a considerate one: the pause also gates
  // MoonMission.stepRover (the `isAnimating && !pausedPlanets` guard further
  // down), so opening VIPER's publication left every drive button dead. And
  // tapping VIPER runs enterRoverMode() first and showDetail() second, so the
  // pause landed last and undid the forced play. cameraFollowTarget re-reads
  // the body's position every frame, so there is nothing to freeze for.
...
function initMoonSurface() {
  ...
  // VIPER rover (v2 model with grouser wheels)
  ...
  // Earthrise: the textured Earth hanging in the black sky
  moonEarth = PlanetsReal.createPlanet({ name: 'Earth', size: 8 });
...
bodyData.forEach(d => {
  ...
  if (d.isStation) {
    ...
    mesh = PlanetsReal.createPlanet(d);
  }
  ...
  if (d.hasRing) {
    PlanetsReal.addRing(mesh, d.size);
  }
});
...
// tilt, wheel spin, dust and wheel tracks (moon-mission.js). Runs for both
// POV driving and arrow-key driving from the orbit view.
if ((roverPOVMode || viperManualControl) && moonViper) {
...
moonCanvas.addEventListener('mousemove', e => {
  ...
  if (moonEarth) PlanetsReal.setHighlight(moonEarth, false);
